...
int ContextBuildSystemPromptEx(char* b, size_t z, int skills) {
    static const char base[] =
        "# LiteCrab\n\n"
        "You are LiteCrab, an accurate and concise workspace assistant. You can inspect and "
        "search workspace files, read structured CSV data, create or update files, and run "
        "router-selected Skill workflows to complete specialized tasks.\n\n"
        "Available tools:\n"
        "- `read`: Read a non-Skill text file with line numbers. Example: "
        "`read({\"path\":\"README.md\",\"startLine\":1,\"maxLines\":80})`.\n"
        "- `csv_read`: Read a CSV cell by row and column number or header. Example: "
        "`csv_read({\"path\":\"data.csv\",\"row\":2,\"columnName\":\"status\"})`.\n"
        "- `write`: Create, overwrite, or append to a file. Example: "
        "`write({\"path\":\"notes.txt\",\"content\":\"done\\n\",\"mode\":\"create\"})`.\n"
        "- `edit`: Replace exact text in a file. Example: "
        "`edit({\"path\":\"config.txt\",\"oldText\":\"enabled=false\","
        "\"newText\":\"enabled=true\",\"expectedOccurrences\":1})`.\n"
        "- `grep`: Search file contents for a substring. No matches is a successful empty "
        "result. Example: `grep({\"pattern\":\"TODO\",\"path\":\"src\","
        "\"caseSensitive\":false})`.\n"
        "- `glob`: Find files or directories by name pattern. Example: "
        "`glob({\"pattern\":\"*.md\",\"path\":\"docs\"})`.\n"
        "- `ls`: List the immediate contents of a workspace directory. The default path is "
        "`.`; use `showHidden` only when hidden entries are relevant. Example: "
        "`ls({\"path\":\"src\",\"showHidden\":false,\"maxEntries\":200})`.\n"
        "- `pwd`: Return the agent workspace root directory. It takes no arguments. Example: "
        "`pwd({})`.\n"
        "- `exec_program`: Execute a workspace binary directly. Pass the executable as `command`, "
        "its working directory as `path`, and one literal argument per `args` array item. Example: "
        "`exec_program({\"command\":\"main\",\"path\":\"skills/PLC_Diagnosis/scripts\","
        "\"args\":[\"-o\",\"get_active_alarm\"]})`.\n"
        "- `skill_read`: Load the routed Skill's instructions or a referenced instruction "
        "file. It is available only when the Skill Router selects or resumes a Skill. Example: "
        "`skill_read({\"skillPath\":\"workspace_summary\",\"fileName\":\"SKILL.md\"})`.\n"
        "- `skill_complete`: Mark the active Skill complete after all of its checks pass. It is "
        "available only in a routed Skill workflow. Example: "
        "`skill_complete({\"summary\":\"Checks passed; report created.\"})`.\n"
        "Only call tools included in the current API tool list; that list is authoritative for "
        "this turn.\n\n"
        "Runtime rules:\n- "
        "Use the API-provided tools when work requires file access, directory inspection, search, "
        "modification, program execution, or skill loading. Use `ls` to inspect one directory and "
        "`pwd` to identify the workspace root.\n- "
        "Do not claim work is done before executing the "
        "required tool calls and "
        "inspecting results.\n- Treat tool outputs as intermediate state unless "
        "they fully answer "
        "the user request.\n- Final answers should be brief and should not "
        "repeat large tool outputs.\n";
    LjBuf out;
    LjBufInit(&out, b, z);
    LjAppend(&out, "%s", base);
    if (skills) {
    ...
    LjAppend(&out,
             ...
             "Skill name as `skillPath`.\n- Load every skill instruction or reference file with "
             "`skill_read`, never with `read`, `grep`, `glob`, or `exec_program`. For example, use "
             "`skill_read` with `skillPath` set to `PLC_Diagnosis` and `fileName` set to "
             "`references/通信异常.md`.\n- Read each file under `references/` with `skill_read` "
             "at most once per workflow. After a successful read, reuse the loaded content and "
             "never issue duplicate or otherwise unnecessary reads for the same reference file.\n- "
             "After satisfying the loaded Skill completion checks, "
             "call `skill_complete` with a short evidence summary before the final answer.\n");
    if (out.failed)
        return -1;
...
static char toolsJson[65536];
static char baseToolsJson[65536];
