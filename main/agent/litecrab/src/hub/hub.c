...
typedef struct {
    SchedulerLane control, urgent, normal;
    int ready, normalsSinceUrgent;
    pthread_mutex_t mutex;
    pthread_cond_t notEmpty, notFull;
} Scheduler;
...
static void scheduler_init(Scheduler* s) {
    ...
    s->urgent.capacity = LITE_URGENT_QUEUE_LEN;
...
static SchedulerLane* select_lane(Scheduler* s, const LiteMsg* m) {
    if (m->type != LITE_MSG_CHAT)
        return &s->control;
    if (m->priority >= LITE_PRIORITY_HIGH)
        return &s->urgent;
    return &s->normal;
}
...
static int scheduler_pop(LiteMsg* m) {
    if (!scheduler.ready)
        MessageBusInit();
    pthread_mutex_lock(&scheduler.mutex);
    while (!scheduler.control.count && !scheduler.urgent.count && !scheduler.normal.count)
        pthread_cond_wait(&scheduler.notEmpty, &scheduler.mutex);
    if (scheduler.control.count) {
        lane_pop(&scheduler.control, m);
    } else if (scheduler.urgent.count &&
               (!scheduler.normal.count || scheduler.normalsSinceUrgent >= 2)) {
        lane_pop(&scheduler.urgent, m);
        scheduler.normalsSinceUrgent = 0;
    } else {
        lane_pop(&scheduler.normal, m);
        scheduler.normalsSinceUrgent++;
    }
