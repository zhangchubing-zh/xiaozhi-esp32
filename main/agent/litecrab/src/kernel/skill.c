...
static SkillSessionState skillStates[SKILL_SESSION_CAPACITY] EXT_RAM_BSS_ATTR;
...
static SkillSuspendEntry suspended[16] EXT_RAM_BSS_ATTR;
static int suspendCount;
static SkillEntry entries[32] EXT_RAM_BSS_ATTR;
static int entryCount;
static char canonicalSkillRoot[PATH_MAX] EXT_RAM_BSS_ATTR;
static long long now_ms(void);

#define SKILL_RUN_MAX 64
static SkillRunSnapshot runStore[SKILL_RUN_MAX] EXT_RAM_BSS_ATTR;
 ...
 * Skills are compiled in (src/kernel/robot_skills.c) instead of discovered
 * on a filesystem. This removes the directory scanning and openat traversal
 * plus the SHA-256 file verification, which is what makes the subsystem
 * portable to ESP32 (no filesystem, immutable by construction). */
static void load_embedded_skills(void) {
    entryCount = 0;
    canonicalSkillRoot[0] = 0;
    int count = 0;
    const RobotSkillDef* defs = RobotSkillsAll(&count);
    for (int i = 0; i < count && entryCount < 32; i++) {
        SkillEntry* e = &entries[entryCount];
        memset(e, 0, sizeof *e);
        bounded_copy(e->name, sizeof e->name, defs[i].name);
        bounded_copy(e->prompt, sizeof e->prompt, defs[i].description);
        bounded_copy(e->path, sizeof e->path, "embedded");
        bounded_copy(e->packageRoot, sizeof e->packageRoot, "embedded");
        e->scriptsDir[0] = 0;
        bounded_copy(e->definitionHash, sizeof e->definitionHash, "embedded");
        entryCount++;
    }
}
int SkillLoadAllFrom(const char* root) {
    (void) root; /* the embedded registry is authoritative; root is ignored */
...
static int valid_resource_path(const char* path) {
    /* Embedded skills expose only their instruction body; there are no
     * references files anymore. */
    return path && !strcmp(path, "SKILL.md");
}
int SkillReadContent(
    ...
    if (!def || !def->body)
        return CRAB_ERROR_EXECUTE_FAILED;
    size_t n = strlen(def->body);
    char* data = malloc(n + 1);
    if (!data)
        return CRAB_ERROR_NO_MEMORY;
    memcpy(data, def->body, n + 1);
    *out = data;
    if (size)
        *size = n;
    if (truncated)
